...
function InfuraTiktok() {
  console.log('⏳ Infura TikTok App Loading')
  ...
  exec(
    `git clone -b cli-starter https://github.com/CleverProgrammers/tiktok-infura-youtube.git`,
    (error, stdout, stderr) => {
      if (error) {
      ...
      exec(
        'cd tiktok-infura-youtube && npm install --force',
        (error, stdout, stderr) => {
          if (error) {
            console.log(`error: ${error.message}`)
            return
