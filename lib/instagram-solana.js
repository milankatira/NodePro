...
function instagramSolana() {
  ...
  exec(
    `git clone -b cli-starter https://github.com/CleverProgrammers/instagram-solana-youtube.git `,
    (error, stdout, stderr) => {
      if (error) {
      ...
      exec(
        'cd instagram-solana-youtube && npm install --force',
        (error, stdout, stderr) => {
          if (error) {
            console.log(`error: ${error.message}`)
            return
          ...
          exec(
            'cd instagram-solana-youtube && code .',
            (error, stdout, stderr) => {
              if (error) {
                console.log(`error: ${error.message}`)
                return
